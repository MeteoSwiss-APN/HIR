...
\element{AbsolutFieldAccess}
The {\tt AbsolutFieldAccess} is a expression that defines a direct access to a grid point of a field, 
...
\begin{lstlisting}[style=default,frame=none]
(^\concept{Literal|VarAccess}^+)
\end{lstlisting}

\begin{HIRChildElements} 
	\HIRElementDef{Offset}
	{An offset (relative to current grid position) used to de-reference the field access}{A}
\end{HIRChildElements}
\begin{HIRAttributes}
	\HIRAttrDef{name}{string}
	{The name of the field declaration that is being accessed in this expression}{R}
...
A reduction operation over near neighbours of certain  \concept{LocationType}.
...
\begin{HIRChildElements} 
	...
	\HIRElementDef{Literal}{the initial value of the reduction}
