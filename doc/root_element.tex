...
The {\tt Dimension} elements defines a dimension of a multidimensional space where fields are discretized and over which \concept{Computation}s iterate. 
...
\begin{lstlisting}[style=default]
(^\concept{Type}^,^\concept{Dimension}^+)
\end{lstlisting}
...
\element{Computation}
The {\tt Computation} defines an iteration loop over the specified \concept{Dimension}s of the domain. 
...
\begin{HIRChildElements}
	\HIRElementDef{Dimension[dimension]}
